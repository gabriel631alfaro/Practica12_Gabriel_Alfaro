...
//KeyFrames
float dogPosX , dogPosY , dogPosZ  ; //posición de los keyframes

#define MAX_FRAMES 9 //NUMERO MAXIMO DE KEYFRAMES
int i_max_steps = 190; // PASOS DE FRAME
int i_curr_steps = 0; //ACTUAL 
typedef struct _frame { //se guardan os calores 
	
	float rotDog;
	float rotDogInc;//incremento o cambio de keyframe 
	float dogPosX;
	float dogPosY;
	float dogPosZ;
	float incX;
...
bool play = false;  //inicio de animacion
int playIndex = 0; // paso de laanimación en la que se encuentra 

void saveFrame(void) //guardar info delos Kayframe 
{
...
void resetElements(void) //reinicializar la posiicón inicial 
{
...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Previo12. Animación por KeyFrames. Gabriel Alfaro", nullptr, nullptr);
	...
	//KeyFrames
	for (int i = 0; i < MAX_FRAMES; i++)//inicializar los keyframes 
	{
