...
int main() {
	...
	GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Practica6_Gabriela_Suarez", nullptr, nullptr);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		color = glm::vec3(2.0f, 1.0f, 0.0f); //solo da el color 
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		...
		modelTemp = model = glm::translate(model, glm::vec3(1.0f, 0.0f, 0.0f));//pibote debe de estar en medio en este caso del 2 que es el eje de las X por eso es 1 en X
		model = glm::scale(model, glm::vec3(2.0f, 1.0f, 1.0f)); //Este 2 es del que hablo arriba
		color = glm::vec3(1.0f, 0.0f, 1.0f); //solo da el color
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		...
		model = glm::scale(model, glm::vec3(0.5f, 1.0f, 1.0f)); //Este 2 es del que hablo arriba
		color = glm::vec3(1.0f, 3.0f, 1.0f); //solo da el color
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		...
		model = glm::scale(model, glm::vec3(1.0f, 0.3f, 0.25f)); //Este 2 es del que hablo arriba
		color = glm::vec3(0.0f, 4.0f, 1.0f); //solo da el color
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		...
		model = glm::scale(model, glm::vec3(1.0f, 0.3f, 0.25f)); //Este 2 es del que hablo arriba
		color = glm::vec3(0.0f, 0.0f, 1.0f); //solo da el color
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
 ...
 void Inputs(GLFWwindow *window) {
	 ...
	 if (glfwGetKey(window, GLFW_KEY_F) == GLFW_PRESS)
		 hombro -= 0.18f;
	 if (glfwGetKey(window, GLFW_KEY_G) == GLFW_PRESS)//nueva codo
		 codo += 0.18f;
	 if (glfwGetKey(window, GLFW_KEY_H) == GLFW_PRESS)//nueva codo
		 codo -= 0.18f;
	 if (glfwGetKey(window, GLFW_KEY_M) == GLFW_PRESS)//nueva muñeca
		 muñeca += 0.18f;
	 if (glfwGetKey(window, GLFW_KEY_N) == GLFW_PRESS)//nueva muñeca
		 muñeca -= 0.18f;
